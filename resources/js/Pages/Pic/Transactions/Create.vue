  ...
  <AppLayout title="Tambah Transaksi">
    <div class="max-w-2xl mx-auto">
      <nav class="flex items-center space-x-2 text-sm text-gray-500 mb-5">
        ...
        <Link :href="route('pic.disbursements.show', disbursement.id)" class="hover:text-green-700 truncate max-w-32">{{ disbursement.name }}</Link>
        <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 5l7 7-7 7"/></svg>
      ...
      <div class="card p-4 mb-5 bg-gradient-to-r from-green-50 to-emerald-50 border-green-200 animate-slide-down">
        <div class="flex items-center justify-between">
          <div>
            <p class="text-xs text-green-600 font-medium">Pencairan Aktif</p>
            <p class="text-sm font-bold text-green-900 mt-0.5">{{ disbursement.name }}</p>
            <p class="text-xs text-green-600">{{ formatDate(disbursement.start_date) }} – {{ formatDate(disbursement.end_date) }}</p>
          </div>
        ...
        <form @submit.prevent="submit" class="space-y-5" enctype="multipart/form-data">
          <FormField label="Tanggal Transaksi" :error="form.errors.transaction_date" required>
            <input v-model="form.transaction_date" type="date" class="form-input"
                   :min="disbursement.start_date" :max="disbursement.end_date" />
            <p class="text-xs text-gray-400 mt-1">Rentang: {{ formatDate(disbursement.start_date) }} – {{ formatDate(disbursement.end_date) }}</p>
          </FormField>
          ...
          <FormField label="Jumlah (Rp)" :error="form.errors.amount" required>
            ...
            <!-- Balance projection -->
            <div v-if="form.amount && form.type" :class="['mt-2 p-2.5 rounded-lg text-xs', projectedBalance < 0 ? 'bg-red-50 text-red-700' : 'bg-green-50 text-green-700']">
          ...
          <div class="flex flex-col sm:flex-row gap-3 pt-2">
            ...
            <Link :href="route('pic.disbursements.show', disbursement.id)" class="btn-secondary text-center">Batal</Link>
          </div>
        </form>
      </div>
    </div>
...
function clearFile() {
  previewFile.value = null; form.proof = null
