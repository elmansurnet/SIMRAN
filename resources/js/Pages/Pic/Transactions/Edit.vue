  ...
  <AppLayout title="Edit Transaksi">
    <div class="max-w-xl mx-auto">
      ...
      <div v-if="disbursement.status === 'grace'"
        ...
        <span>Periode persiapan laporan — transaksi masih bisa diedit.</span>
      </div>
        ...
        <form @submit.prevent="submit" class="space-y-5">
          ...
          <FormField label="Tanggal Transaksi" :error="form.errors.transaction_date" required>
            <input v-model="form.transaction_date" type="date" class="form-input"
                   :min="disbursement.start_date" :max="maxDate" />
          </FormField>
          ...
          <FormField label="Jumlah (Rp)" :error="form.errors.amount" required>
            <div class="relative">
              ...
              <input v-model="form.amount" type="number" class="form-input pl-10" min="1" step="100" />
            </div>
          </FormField>

          <FormField label="Ganti Bukti (PDF/Gambar, maks 5 MB)" :error="form.errors.proof"
                     :hint="transaction.proof_name ? `Bukti saat ini: ${transaction.proof_name}` : 'Belum ada bukti'">
            <div class="border-2 border-dashed border-gray-200 rounded-xl p-4 text-center cursor-pointer hover:border-green-400 transition"
              ...
              <input ref="proofInput" type="file" class="hidden" accept=".pdf,image/*" @change="handleFile" />
          ...
          <div class="flex gap-3 pt-2">
            ...
            <Link :href="route('pic.disbursements.show', disbursement.id)" class="btn-secondary flex-1 text-center">
              Batal
            </Link>
          </div>
        </form>
...
const props = defineProps({ disbursement: Object, transaction: Object })
const page  = usePage()

const proofInput   = ref(null)
const selectedFile = ref(null)

// Max date = end_date + extra_transaction_days
const maxDate = computed(() => {
  const extra = page.props.extra_transaction_days || 0
  if (!extra) return props.disbursement.end_date
  const d = new Date(props.disbursement.end_date)
...
function handleFile(e) {
  ...
  if (file) { selectedFile.value = file; form.proof = file }
}

function submit() {
  form.put(route('pic.disbursements.transactions.update', [props.disbursement.id, props.transaction.id]))
}
</script>
